...
class RepositoryHygieneTests(unittest.TestCase):
    ...
    def test_readme_documents_remote_container_smoke_gate(self) -> None:
        ...
        self.assertIn("health, upload, status polling, result retrieval, and artifact download", readme)

    def test_public_docs_cover_api_contract_and_evidence(self) -> None:
        readme = (ROOT / "README.md").read_text(encoding="utf-8")
        api = (ROOT / "docs" / "API.md").read_text(encoding="utf-8")
        self.assertIn("[HTTP API](docs/API.md)", readme)
        self.assertIn("[benchmark report](BENCHMARKS.md)", readme)
        self.assertIn("[concurrent load-test report](LOADTEST.md)", readme)
        self.assertIn("private development history", readme)
        self.assertTrue((ROOT / "docs" / "assets" / "synthetic-pressure-stance.png").is_file())
        for token in (
    ...
    def test_mini_program_configuration_has_no_stale_endpoint(self) -> None:
        ...
        project_config = (ROOT / "stepwise_miniprogram" / "project.config.json").read_text(
            encoding="utf-8"
        )
        self.assertIn('"appid": "touristappid"', project_config)
