...
def create_app(
    ...
    *,
    ...
    async def create_analysis(
        ...
        walking_bytes = await _read_upload(walking, resolved_settings.max_upload_bytes)
        standing_bytes = (
            await _read_upload(standing, resolved_settings.max_upload_bytes)
            if standing is not None
            else None
        )
        try:
            manifest = job_manager.submit(walking_bytes, standing_bytes, config)
