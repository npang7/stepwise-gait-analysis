...
These measurements are preserved from an earlier synthetic-data development history.
Recorded source commit IDs refer to a private archive, not to revisions in this public
repository. Numeric values are unchanged. The published concurrent-load dataset
normalizes non-metric background process names; the original is retained privately.
